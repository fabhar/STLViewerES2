...
//	float touchedY = 0;
	...
	private STLObject stlObject = null;

	 GLSurfaceView mActivityContext;		
	/**
	 * Initialize the model data.
	 */
	public Cuberenderer(Cubesurfaceview cubesurfaceview, STLObject stlObject)
	{
...
//effects of projection matrix. if you want you can uncomment this line			
	...
	{
        if (stlObject.isLoadDone) {
            mcubeVertices = ByteBuffer.allocateDirect(stlObject.vertexArray.length * mBytesPerFloat).order(ByteOrder.nativeOrder()).asFloatBuffer();
            mcubeVertices.put(stlObject.vertexArray).position(0);
